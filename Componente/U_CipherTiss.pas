...
uses
  StdCtrls, MD5tiss, IDEAtiss, DCPcrypttiss, Forms,
...
Var Temp : String;
begin
  IDEA_Buffer := TDCP_ideaTiss.Create(Application);
...
begin
  ...
  Cipher:= TDCP_ideaTiss.Create(Application);
  try
  ...
  if Cipher is TDCP_blockciphertiss then
    TDCP_blockciphertiss(Cipher).CipherMode:= cmCFBblock; { Set the appropriate mode if a block cipher }
...
begin
  ...
  if Cipher is TDCP_blockciphertiss then
    TDCP_blockciphertiss(Cipher).CipherMode:= cmCFBblock; { Set the mode }
